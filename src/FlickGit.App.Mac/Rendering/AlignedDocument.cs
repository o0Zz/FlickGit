...
/// </list>
///
/// Filler positions are tracked with <see cref="TextAnchor"/>s rather than line numbers. That is
/// load-bearing: a list of numbers captured when the document was built goes stale the moment the
/// user inserts or removes a line, and the reconstruction would then drop the wrong ones — silently
/// writing a blank line into the file, or silently deleting one the user typed. An anchor moves with
/// its text.
///
/// <b>The arithmetic that produces the file's bytes is not here.</b>
/// <c>DiffDocument.ToFileText</c> is in FlickGit.Core, where Hard Requirement 4 puts "the one value
/// that may ever be written to a file" and where it is tested. This class's job is only to keep the
/// filler line numbers current and hand them over — which is why porting it to a second UI toolkit
/// is an anchor-API question rather than a re-derivation of the dangerous part.
/// </summary>
internal sealed class AlignedDocument(TextEditor editor)
{
    private readonly List<TextAnchor> _fillerAnchors = [];

    /// <summary>
    /// Replaces the document with the pane's text and re-anchors the fillers.
    ///
