...
/// <param name="LeftStart">1-based first line on the left, or 0 when the hunk adds to an empty file.</param>
...
///
/// The viewer diffs two buffers rather than parsing <c>git diff</c> output -- that is what lets
/// the right pane be edited -- so there is no patch lying around to stage from.
///
/// <b>The line endings are the whole difficulty.</b> <c>git apply</c> matches context lines
/// against the index byte for byte, and in a CRLF file the content of a line <i>includes</i> its
/// carriage return. A patch built from the normalised text this viewer holds would be rejected on
/// every line of every CRLF repository -- so each emitted line is re-terminated from the
/// <see cref="FileText"/> it came from, per line for a file whose endings are mixed.
/// </summary>
public static class Hunks
{
    /// <summary>
    /// Context lines kept either side of a change. Three, which is what every Git tool emits and
    /// therefore what <c>git apply</c> is most forgiving about: fewer makes a patch that fails to
    /// locate itself once the index has moved on, more merges neighbouring changes into one hunk the
    /// user cannot stage separately.
    /// </summary>
    private const int ContextLines = 3;

    /// <summary>
    /// Every hunk in <paramref name="rows"/>, in order. Two changes closer together than twice the
    /// context become one hunk, because their context would otherwise overlap and a patch cannot
    /// describe the same line twice.
    /// </summary>
    ...
    /// <summary>
    /// A patch that stages exactly the changed rows in <paramref name="staged"/>. One function serves
    /// whole-hunk and selected-line staging, because they differ only in which rows are in the set.
    ///
    /// Rows inside the window that are <i>not</i> staged are demoted rather than dropped, and the two
    /// directions are not symmetric: an unstaged <b>deletion</b> becomes a context line, because it
    /// is present on both sides of this patch; an unstaged <b>insertion</b> is omitted entirely,
    /// because it is on neither. The wrong way round produces a patch that applies cleanly and stages
    /// the opposite of what the user picked.
    /// </summary>
        ...
        FileText right)
    {
        ...
        for (int row = windowStart; row <= windowEnd; row++)
        {
            ...
            if (!IsChange(current))
            {
                //A file that ends with a newline produces one row past its last line: the empty string after the
                //final terminator. Emitting it as context makes the hunk header claim one line more than the
                //file has, which git rejects as "patch does not apply", at the end of every file.
    ...
    /// <summary>
    /// Whether <paramref name="row"/> represents a difference. A filler row is padding the renderers
    /// use to keep the panes aligned.
    ///
    ...
    /// <c>Ctrl+Z</c> undoes it and nothing has touched the disk -- which is what makes "discard my
    /// work" safe to offer on one click.
    ///
    /// It works in the normalised text the viewer holds and says nothing about line endings.
    /// <see cref="ToPatch"/> has to re-terminate every line because <c>git apply</c> compares bytes;
    /// this does not, because <c>WorkingTreeWriter</c> restores the file's own encoding, BOM and
    /// endings when it saves. Two places deciding line endings is how a one-line revert becomes a
    /// whole-file diff.
    ///
    /// Every row is emitted in order: a selected change contributes its <b>left</b> side, everything
    /// else its <b>right</b>. A side that is filler contributes no line at all, which is what makes
    /// both directions fall out of one rule -- reverting an added line drops it, reverting a deleted
    /// line brings it back.
    /// </summary>
    /// <param name="rows">
    ...
    /// changed.
    /// </param>
    /// <returns>The complete new right-hand text, or null when the selection contains no change.</returns>
    public static string? RevertRows(IReadOnlyList<DiffRow> rows, IReadOnlySet<int> selected)
    {
        ...
        for (int row = 0; row < rows.Count; row++)
        {
            ...
            //Filler contributes nothing. That is the whole of the asymmetry between reverting an insertion
            //and reverting a deletion.
            if (!side.IsFiller)
                lines.Add(side.Text);
        }

        //Joined with \n and nothing else. A file that ends with a newline arrives here as a final empty
        //row, so the join reproduces that terminator without this having to know whether the file had
        //one -- and a file that does not, does not gain one.
        return string.Join('\n', lines);
    }

    /// <summary>
    /// Whether <paramref name="lineNumber"/> names a line the file actually has. The differ works on
    /// text split by newline, so a file ending with one yields a final empty element that is not a
    /// line -- and every patch whose context reaches the bottom of a file depends on it being dropped.
    /// </summary>
    ...
    /// <c>git apply</c> compares a context line to the index byte for byte, so the patch has to carry
    /// the carriage return or nothing matches.
    /// </summary>
    private static void Emit(StringBuilder body, char prefix, DiffSide side, FileText file)
    {
        ...
        {
            //The file's last line is unterminated. Without this marker git apply would add a newline the
    ...
    /// <summary>
    /// What terminated line <paramref name="lineNumber"/>. Empty means the line was the last and had
    /// no terminator. A mixed-ending file is answered per line, because rewriting one kind as the
    /// other is exactly the whole-file diff <see cref="FileText"/> exists to prevent.
    /// </summary>
